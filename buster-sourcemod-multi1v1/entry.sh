...
if [ ! -f "${STEAMAPPDIR}/${STEAMAPP}/cfg/server.cfg" ]; then
	...
	# Using Multi 1v1 Plugin
	if [ ! -z "$MULTI1v1_VERSION" ]; then
		wget --max-redirect=30 "${DOWNLOAD_URL}" -O "${STEAMAPPDIR}/plugin.zip"
		unzip "${STEAMAPPDIR}/plugin.zip" -d "${STEAMAPPDIR}/csgo"
		rm "${STEAMAPPDIR}/plugin.zip"
	fi
